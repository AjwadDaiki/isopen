...
import { t, type Locale, LOCALES } from "@/lib/i18n/translations";
...
const TARGET_LOCALES: Locale[] = ["fr", "es", "de"];
...
export async function generateStaticParams() {
  const params: Array<{ locale: Locale; slug: string; category: string }> = [];
  ...
  for (const locale of TARGET_LOCALES) {
    for (const citySlug of getAllCitySlugs()) {
      ...
      for (const focusCategory of city.focusCategories) {
        ...
        seen.add(focusSlug);
        params.push({ locale, slug: citySlug, category: focusSlug });
      }
    }
  }
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!TARGET_LOCALES.includes(loc)) return { title: "Not Found" };
  ...
  const pathByLocale: Partial<Record<Locale, string>> = {
    en: `/city/${city.slug}/category/${categorySlug}`,
    fr: `/fr/city/${city.slug}/category/${categorySlug}`,
    es: `/es/city/${city.slug}/category/${categorySlug}`,
    de: `/de/city/${city.slug}/category/${categorySlug}`,
  };
  const alternates = buildLocaleAlternates(pathByLocale);
  const canonicalPath = pathByLocale[loc] || pathByLocale.en!;

  return {
    title: `${category} ${t(loc, "open")} - ${city.name}, ${city.state}`,
    description: `${t(loc, "categoryDesc", { category: category.toLowerCase() })} ${city.name}, ${city.state}.`,
    alternates: {
    ...
    openGraph: {
      ...
      title: `${category} ${t(loc, "open")} - ${city.name}`,
      description: `${t(loc, "categoryDesc", { category: category.toLowerCase() })} ${city.name}.`,
    },
  };
}

export default async function LocaleCityCategoryPage({ params }: PageProps) {
  const { locale, slug, category: categorySlug } = await params;
  const loc = locale as Locale;
  if (!TARGET_LOCALES.includes(loc) || !LOCALES.includes(loc)) notFound();
  ...
  const otherCities = getCitiesForCategory(category, 10).filter((entry) => entry.slug !== city.slug).slice(0, 8);
  ...
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    ...
    { name: t(loc, "cityNav"), item: absoluteUrl("/city") },
    { name: `${city.name}, ${city.state}`, item: absoluteUrl(`/city/${city.slug}`) },
    { name: category, item: absoluteUrl(`/${loc}/city/${city.slug}/category/${categorySlug}`) },
  ]);
      ...
      <div className="min-h-screen">
        ...
        <div className="page-pad">
          <nav className="breadcrumb-row">
            ...
            <span>/</span>
            <Link href="/city" className="text-muted2 no-underline hover:text-text transition-colors">{t(loc, "cityNav")}</Link>
            <span>/</span>
        ...
        <div className="page-pad pt-4 pb-16">
          <div className="content-grid-shell">
            <main className="min-w-0 content-stack">
              <section className="ui-panel overflow-hidden">
                <div className="panel-body-lg">
                  <p className="font-mono uppercase tracking-[0.12em] text-[11px] text-muted mb-3">{t(loc, "realTimeStatus")}</p>
                  <h1 className="font-heading font-extrabold text-[30px] sm:text-[42px] tracking-[-0.04em] leading-[0.95] text-text">
                    {category} - {city.name}, {city.state}
                  </h1>
                  ...
                  <div className="mt-6 inline-flex items-center gap-2.5 rounded-full px-4 py-2.5 border ui-border-green-30 bg-green-dim text-[12px] text-green font-semibold">
                    ...
                    {openCount} / {statuses.length} {t(loc, "openNowLabel")}
                  </div>
                </div>
              </section>
              ...
              <section className="ui-panel overflow-hidden">
                <div className="card-title-row">
                  <h2 className="font-heading font-bold text-[15px] text-text tracking-[-0.01em]">{t(loc, "topBrands")}</h2>
                </div>
                  ...
                  {statuses.map(({ brand, status }, i) => (
                    ...
                    >
                      ...
                      <p className="text-[12px] text-muted2">
                        {status.isOpen && status.closesIn ? `${t(loc, "closesIn")} ${status.closesIn}` : ""}
                        {!status.isOpen && status.opensAt ? `${t(loc, "opensAt")} ${status.opensAt}` : ""}
                      </p>
                    </Link>
                  ))}
                </div>
            ...
            <aside className="sidebar-stack">
              <section className="ui-panel overflow-hidden">
                <div className="card-title-row">
                  <h2 className="font-heading font-bold text-[15px] text-text tracking-[-0.01em]">{category} {t(loc, "cityNav").toLowerCase()}</h2>
                </div>
                <div className="panel-body flex flex-col gap-3">
                  {otherCities.map((entry) => (
                    <Link
                      key={entry.slug}
                      href={`/${loc}/city/${entry.slug}/category/${categorySlug}`}
                    ...
                    >
                      ...
                      <p className="text-[12px] text-muted2 mt-1">{t(loc, "realTimeStatus")}</p>
                    </Link>
                  ))}
                </div>
              </section>
